...
package com.movtery.cytechlauncher.game.version.installed

import androidx.compose.runtime.Composable
import androidx.compose.ui.res.stringResource
import com.movtery.cytechlauncher.R
import com.movtery.cytechlauncher.game.path.getVersionsHome
import com.movtery.cytechlauncher.game.version.installed.utils.parseJsonToVersionInfo
...
object VersionsManager {
    ...
    /**
     * @return 获取 Cytech 启动器版本标识文件夹
     */
    fun getCytechVersionPath(version: Version) = File(version.getVersionPath(), InfoDistributor.LAUNCHER_IDENTIFIER)

    /**
     * @return 通过目录获取 Cytech 启动器版本标识文件夹
     */
    fun getCytechVersionPath(folder: File) = File(folder, InfoDistributor.LAUNCHER_IDENTIFIER)

    /**
     * @return 通过名称获取 Cytech 启动器版本标识文件夹
     */
    fun getCytechVersionPath(name: String) = File(getVersionPath(name), InfoDistributor.LAUNCHER_IDENTIFIER)

    /**
     * @return 获取当前版本设置的图标
     */
    fun getVersionIconFile(version: Version) = File(getCytechVersionPath(version), "VersionIcon.png")

    /**
     * @return 通过目录获取 Cytech 启动器版本标识文件夹
     */
    fun getVersionIconFile(folder: File) = File(getCytechVersionPath(folder), "VersionIcon.png")

    /**
     * @return 通过名称获取当前版本设置的图标
     */
    fun getVersionIconFile(name: String) = File(getCytechVersionPath(name), "VersionIcon.png")
