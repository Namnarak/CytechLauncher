...
package com.movtery.cytechlauncher.bridge
...
import com.movtery.cytechlauncher.utils.killProgress
        ...
        (GlobalContext as? Activity)?.let { activity ->
            activity.runOnUiThread {
                val clipData = (activity.getSystemService(CLIPBOARD_SERVICE) as? ClipboardManager)?.primaryClip ?: run {
                    CytechBridge.clipboardReceived(null, null)
                ...
                CytechBridge.clipboardReceived(clipItemText.toString(), "plain")
            }
        }
    }
